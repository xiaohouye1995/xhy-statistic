export class XhyStatistic {
  constructor() {
    ...
    if (id) {
      ...
      localStorage.setItem('xhyStatisticBrowserId', this.browserId)
    }
    this.previous = 0
    this.routerInit()
  }
  ...
  // 获取页面来源
  getPageSource() {
    const source = localStorage.getItem('xhyStatisticSource')
    if (source) {
      this._options.source = source
    } else {
      this._options.source = this.getQueryVariable('source') || document.referrer || '直接打开'
      localStorage.setItem('xhyStatisticSource', this._options.source)
    }
  }
  // 获取url参数
  ...
  getPageOut() {
    ...
    // 从缓存中获取用户上次退出的时间戳
    const leaveTime = parseInt(localStorage.getItem('xhyStatisticLeaveTime'), 10)
    if (!leaveTime) {
      return
    }
    ...
    if (isOut) {
      const url = localStorage.getItem('xhyStatisticPageUrl') || this.getUrl()
      this.send({ ...this._options, eventType: 'pageOut', pagePath: url })
      localStorage.setItem('xhyStatisticSource', '')
    }
  }
  // 获取页面返回上一页
  getPageBack() {
  ...
  // 获取页面停留时长
  getPageTime() {
    let tempTime = localStorage.getItem('xhyStatisticTime') || new Date().getTime()
    let pageUrl = localStorage.getItem('xhyStatisticPageUrl') || this.getUrl()
    const setStayTimeEvent = (name, path) => {
      ...
      localStorage.setItem('xhyStatisticTime', tempTime)
      localStorage.setItem('xhyStatisticPageUrl', pageUrl)
    ...
    window.onunload = () => {
      ...
      // 清空停留时长和停留页面
      localStorage.setItem('xhyStatisticTime', '')
      localStorage.setItem('xhyStatisticPageUrl', '')
      // 记录离开时间，用以区分刷新和退出
      localStorage.setItem('xhyStatisticLeaveTime', new Date().getTime())
    }
